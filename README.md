...
3. Set User-Delegated Application Access to per-app authorization and grant ChatGPT's dynamically registered client access to the API. Set `JWT_CLIENT_IDS` to that client's `tpc_...` ID so tokens from other OAuth clients are rejected. Deploy the included Auth0 Post Login Action so only that client and the CrossLLM audience receive the `run:panel` scope.
